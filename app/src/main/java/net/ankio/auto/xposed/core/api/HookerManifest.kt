...
abstract class HookerManifest : ILogger by Logger {
    ...
    fun permissionCheck() {
        ...
        permissions.forEach {
            ...
            e(Throwable("$appName 权限被拒绝: $it，此Hook可能无法正常工作"))
        }
    }
    ...
    fun versionCheck(): Boolean {
        ...
        i("应用版本号: $code, 版本名: $name")

        // 检查App版本是否过低，过低无法使用
        if (minVersion != 0 && code < minVersion) {
            e(Throwable("自动适配失败，${AppRuntime.manifest.appName}(${code}) 版本过低"))
    ...
    fun autoAdaption(rules: MutableList<Clazz>): Boolean {
        ...
        d("适配版本: $savedVersion, 规则哈希: $savedRulesHash")
            ...
            try {
                ...
                try {
                    ...
                    val hashMap = Dex.findClazz(
                        ...
                        d("适配进度: $found/$total, 找到规则: $ruleName")
                    }
    ...
    fun analysisData(type: DataType, data: String, appPackage: String = packageName) {
        ThreadUtils.launch {
            ...
            if (filter.all { !data.contains(it) }) {
                i("all filter not contains: $data, $filter")
                return@launch
            }
            ...
            while (result == null && retryCount < 10) {
                ...
                if (result == null) {
                    ...
                    d("分析尝试第 $retryCount 次失败，将在 $delaySeconds 秒后重试...")
