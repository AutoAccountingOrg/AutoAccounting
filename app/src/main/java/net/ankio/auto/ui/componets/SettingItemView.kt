...
class SettingItemView @JvmOverloads constructor(
    ...
    init {
        ...
        // 获取自定义属性
        context.obtainStyledAttributes(attrs, R.styleable.SettingItemView).use { typedArray ->
            // 设置图标
            val iconRes = typedArray.getResourceId(R.styleable.SettingItemView_settingIcon, 0)
            if (iconRes != 0) {
                binding.settingIcon.setImageResource(iconRes)
            }
            
            // 设置标题
            binding.settingTitle.text = typedArray.getString(R.styleable.SettingItemView_settingTitle)
            
            // 设置描述
            binding.settingDesc.text = typedArray.getString(R.styleable.SettingItemView_settingDesc)
