...
object Dex {
     ...
     * 查找某个字段，只能根据类型查找咯
     */
   fun findField(clazz: Class<*>, field: ClazzField): String {
        //判断指定要求的字段是否在class里面
    ...
    private fun findFieldIsExist(clazz: Class<*>, field: ClazzField): Boolean {
        ...
        for (f in fields) {
            ...
            val condition1 = if (field.name.isNotEmpty()) {
                f.name == field.name
            } else {
                true
            }

            val condition2 = if (field.type.isNotEmpty()) {
     ...
     * 查找某个方法
     */
    fun findMethod(clazz: Class<*>, clazzMethod: ClazzMethod): String {
        //判断指定要求的字段是否在class里面
        ...
        for (method in methods) {
            ...
            val condition3 = if (clazzMethod.modifiers.isNotEmpty()) {
                method.modifiers.toString() == clazzMethod.modifiers
            } else {
                true
            }

            val condition4 = if (clazzMethod.parameters.isNotEmpty()) {
