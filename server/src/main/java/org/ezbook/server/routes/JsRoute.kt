...
class JsRoute(private val session: IHTTPSession, private val context: android.content.Context) {
    ...
    fun analysis(): Response {
        ...
        try {
            ...
            return Server.json(400, "type is required")
        }
        ...
        if (result === "") {
            return Server.json(404, "analysis nothing")
        }
     ...
     * 运行js代码
     */
    private fun runJS(jsCode:String):String{
        val code  = jsCode.replace("print","console.log")
        Server.log(code)
        ...
        try {
            ...
            result = rhino.evaluateString(scope, code, "JavaScript", 1, null)
        } catch (e: Throwable) {
            Server.log(e)
        } finally {
            Context.exit()
